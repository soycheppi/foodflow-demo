...
import { useAllProducts } from '@/features/catalog-filters/catalogHooks';
import Drawer from '@/shared/ui/Drawer';
import { t } from '@/config/locales';

interface CartDrawerHandle {
  show: () => void;
  hide: () => void;
}

const CartDrawer = forwardRef<CartDrawerHandle, {}>((_props, ref) => {
  const articulos = useCartStore((s) => s.articulos);
  const removerDelCarrito = useCartStore((s) => s.removerDelCarrito);
  ...
  useEffect(() => {
    if (!isCartOpen || !allProducts) return;
    const currentItems = useCartStore.getState().articulos;
    if (currentItems.length === 0) return;

    const report = reconcileCartWithCatalog(currentItems, allProducts);
    if (report.hasPriceChanges || report.hasAvailabilityIssues) {
      useCartStore.getState().setItems([...report.reconciledItems]);

      const notices: string[] = [];
      if (report.hasPriceChanges) notices.push('Prices updated to current catalog.');
      if (report.hasAvailabilityIssues) notices.push('Some unavailable items were adjusted.');
      setReconcileNotice(notices.join(' '));
    }
  }, [isCartOpen, allProducts]);
  ...
  return (
    ...
    >
      <div className="flex flex-col h-full">
        {reconcileNotice && (
          <div className="mb-4 p-3 bg-brand-red/10 border border-brand-red/20 rounded-xl text-brand-red dark:text-red-400 text-xs font-bold flex items-center justify-between">
            <span>{reconcileNotice}</span>
            <button
              onClick={() => setReconcileNotice(null)}
              className="text-xs uppercase ml-2 underline hover:opacity-80 cursor-pointer"
            >
              OK
            </button>
          </div>
